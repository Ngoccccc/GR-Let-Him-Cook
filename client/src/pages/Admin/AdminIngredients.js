        ...
        },
        {
            ...
            render: (text, record) => (
                <Flex justify="center" gap={10}>
                    ...
                    <Button className="btn btn-danger" onClick={() => handleDeleteIngredient(record)}>
                        Xóa
                    </Button>
                </Flex>
            ),
    ...
    const handleDeleteIngredient = async (ingredient) => {
        try {
            await Modal.confirm({
                ...
                onOk: async () => {
                    await axios.delete(`/api/v1/ingredient/delete-ingredient/${ingredient._id}`);
    ...
    const handleSaveIngredient = async () => {
        try {
            ...
            if (currentIngredient) {
                await axios.put(`/api/v1/ingredient/update-ingredient/${currentIngredient._id}`, values);
                setIngredients(
                    ingredients.map((i) => (i._id === currentIngredient._id ? { ...i, ...values } : i))
                );
            } else {
                const res = await axios.post('/api/v1/ingredient/create-ingredient', values);
